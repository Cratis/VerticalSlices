...
public class with_set_mapping_missing_source_property : Specification
{
    ...
    void Establish()
    {
        ...
        {
            new ReadModelProperty("Name", "string", [new EventPropertyMapping("OrderPlaced", EventPropertyMappingKind.Set)])
        };
    ...
    void Because() => _result = new MissingSourcePropertyInMappingRule().Evaluate(_modules).ToList();
    ...
    [Fact] void should_mention_the_mapping_kind() => _result[0].Message.ShouldContain("Set");
    [Fact] void should_mention_the_property_name() => _result[0].Message.ShouldContain("Name");
}
