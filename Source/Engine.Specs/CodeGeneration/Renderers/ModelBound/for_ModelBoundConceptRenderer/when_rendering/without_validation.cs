...
public class without_validation : given.a_context
{
    ...
    [Fact] void should_emit_implicit_conversion_operator() => _result.Single().Content.ShouldContain("public static implicit operator OrderId(string value) => new(Value: value);");
    [Fact] void should_emit_not_set_default() => _result.Single().Content.ShouldContain("public static readonly OrderId NotSet = new(Value: string.Empty);");
}
