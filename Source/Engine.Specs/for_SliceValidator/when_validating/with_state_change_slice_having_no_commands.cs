...
/// <summary>
/// A StateChange slice must have at least one command. Without a command there is
/// no way to capture user intent, so the validator must reject such a slice.
/// </summary>
public class with_state_change_slice_having_no_commands : Specification
{
    ...
    [Fact] void should_mention_command_in_the_message() => _exception!.Errors[0].Message.ShouldContain("command");
}
