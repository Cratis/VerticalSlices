...
/// <summary>
/// An Add mapping kind accumulates a numeric property from an event onto the read model total.
/// The source property name must be preserved in EventPropertyName, not ContextProperty.
/// </summary>
public class with_add_mapping_kind : Specification
{
    ...
    void Establish()
    {
        var addMapping = new EventPropertyMapping("ItemAdded", EventPropertyMappingKind.Add, "Quantity");

        _readModel = new ReadModel(
            "CartTotals",
            "Cart totals for a customer",
            [new ReadModelProperty("TotalQuantity", "int", [addMapping])]);
    }

    void Because() => _result = ReadModelDescriptor.FromReadModel(
        _readModel,
        [new EventType("ItemAdded", "Item added to cart", [new Property("Quantity", "int")])]);

    [Fact] void should_preserve_add_kind() =>
        _result.Properties.First().Mappings.First().Kind.ShouldEqual(PropertyMappingKind.Add);
